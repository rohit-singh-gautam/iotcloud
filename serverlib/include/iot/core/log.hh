...
#define LOGGER_LOG_LIST \
    \
    LOGGER_ENTRY(END_OF_CLUSTER, IGNORE, "This is end of cluster only parameter is its length") \
    \
    LOGGER_ENTRY(PTHREAD_CREATE_FAILED, ERROR, "Unable to create pthread with error %ve") \
    LOGGER_ENTRY(PTHREAD_JOIN_FAILED, WARNING, "Unable to join pthread with error %ve") \
    LOGGER_ENTRY(EVENT_DIST_CREATE_FAILED, ERROR, "Event distributor creation failed with error %ve, terminating application") \
    LOGGER_ENTRY(EVENT_DIST_CREATE_NO_THREAD, ERROR, "Event distributor failed to create any thread, terminating application") \
    LOGGER_ENTRY(EVENT_DIST_LOOP_WAIT_INTERRUPTED, WARNING, "Event distributor loop interrupted with error %ve,  waiting for a second and retry") \
    LOGGER_ENTRY(EVENT_DIST_TOO_MANY_THREAD, WARNING, "Event distributor created with threads more than CPUs") \
    LOGGER_ENTRY(EVENT_DIST_EXIT_EPOLL_CLOSE_FAILED, WARNING, "Event distributor unable to close epoll with error %ve") \
    LOGGER_ENTRY(EVENT_DIST_NO_THREAD_CANCEL, WARNING, "Event distributor unable to set thread cancel flag with error %ve, exit may not be proper") \
    LOGGER_ENTRY(EVENT_DIST_EXIT_THREAD_CANCEL_FAILED, WARNING, "Event distributor unable to cancel thread with error %ve") \
    LOGGER_ENTRY(EVENT_DIST_EXIT_THREAD_JOIN_FAILED, WARNING, "Event distributor unable to join thread with error %ve") \
    LOGGER_ENTRY(EVENT_DIST_CREATE_SUCCESS, VERBOSE, "Event distributor creation succeeded") \
    LOGGER_ENTRY(EVENT_CREATE_FAILED, ERROR, "Event creation failed with error %ve") \
    LOGGER_ENTRY(EVENT_CREATE_SUCCESS, VERBOSE, "Event creation succeeded") \
    \
    LOGGER_ENTRY(SYSTEM_ERROR, ERROR, "System Error '%ve'") \
    LOGGER_ENTRY(IOT_ERROR, ERROR, "IOT Error '%vE'") \
    LOGGER_ENTRY(TEST_STATE_LOG, INFO, "State '%vs'") \
    LOGGER_ENTRY(TEST_GUID_LOG, INFO, "IOT Error '%vg' caps '%vG'") \
    LOGGER_ENTRY(TEST_FLOAT_LOGS, INFO, "Test float %%%f, double %lf") \
    LOGGER_ENTRY(TEST_INTEGER_LOGS, INFO, "Test %%, Integer %i, long %li, long long %lli, Short %hi, Short Short %hhi, Unsigned %u, long %lu, long long %llu, Short %hu, Short Short %hhu") \
    LOGGER_ENTRY(TEST_IPV6ADDR_LOGS, INFO, "Test char %c, ipv6_socket_addr_t %vn caps: %vN; ipv6_addr_t %vi caps: %vI ipv6_port_t %vp") \
    \
    LOGGER_ENTRY(MAX_LOG, VERBOSE, "Max log no entry must be made beyond this") \
    LIST_DEFINITION_END
...
#define LOGGER_ENTRY(x, y, z) x,
    LOGGER_LOG_LIST
#undef LOGGER_ENTRY
};

template <log_t ID> struct log_description { static constexpr char const value[] = "BAD Message"; };
#define LOGGER_ENTRY(x, y, z) template <> struct log_description<log_t::x> { \
        ...
        log_description<log_t::x>() { static_assert(type_list.type_list_check(), "Bad log string"); } \
    };
    LOGGER_LOG_LIST
#undef LOGGER_ENTRY
...
inline constexpr const char * get_log_description(log_t id) {
    switch (id) {
        default: // This will avoid error, such condition will never reach
            assert(true);
#define LOGGER_ENTRY(x, y, z) case log_t::x: return z;
    LOGGER_LOG_LIST
#undef LOGGER_ENTRY
    }
}
...
inline constexpr size_t get_log_length(log_t id) {
    switch (id) {
        default: // This will avoid error, such condition will never reach
            assert(true);
#define LOGGER_ENTRY(x, y, z) case log_t::x: return log_description<log_t::x>::length;
    LOGGER_LOG_LIST
#undef LOGGER_ENTRY
    }
}
...
inline constexpr const char * get_log_id_string(log_t id) {
    switch (id) {
        default: // This will avoid error, such condition will never reach
            assert(true);
#define LOGGER_ENTRY(x, y, z) case log_t::x: return #x;
    LOGGER_LOG_LIST
#undef LOGGER_ENTRY
    }
}

#define LOGGER_ENTRY(x, y, z) +1
...
public:
    ...
    constexpr logger_level level() const {
        ...
        default: // This will avoid error, such condition will never reach
            assert(true);
#define LOGGER_ENTRY(x, y, z) case log_t::x: return logger_level::y;
            LOGGER_LOG_LIST
#undef LOGGER_ENTRY           
        }
    }
...
private:
    ...
    template<log_t ID, typename... ARGS>
    friend constexpr void log(const ARGS&... args);
