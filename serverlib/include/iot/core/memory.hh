...
struct fixed_memory_alloc_info {
    uint8_t  store_index;
    static constexpr uint8_t default_memory_check = 0xa;
    // This is unique memory can be utilized for memory check in debug mode
    uint8_t  memory_check;
    uint16_t alloc_size;


    constexpr fixed_memory_alloc_info(const uint8_t store_index, const uint16_t alloc_size)
        : store_index(store_index), memory_check(default_memory_check), alloc_size(alloc_size) {}

    constexpr fixed_memory_alloc_info(const fixed_memory_alloc_info &info) 
        : store_index(info.store_index), memory_check(info.memory_check), alloc_size(info.alloc_size) {}
...
struct fixed_memory_free_info {
    ...
    uint32_t store_index:8;
    uint32_t memory_index:24;

    constexpr fixed_memory_free_info(const uint8_t store_index, const uint32_t memory_index)
...
static_assert(sizeof(fixed_memory_alloc_info) == sizeof(fixed_memory_free_info),
    "Size of fixed_memory_alloc_info and fixed_memory_free_info must be equal");
...
private:
    static constexpr fixed_memory_free_info null_index = {0xff, 0xffffff};
...
public:
    inline fixed_memory(const size_t alloc_size)
            :   alloc_size(alloc_size + sizeof(fixed_memory_free_info)),
...
private:
    ...
    inline uint8_t *get_memory(const size_t alloc_size) {
        const size_t chunk_index = alloc_size >> 2;

        uint8_t *memptr = mem_array[chunk_index].get_memory();
...
public:
    ...
    inline void *alloc(const size_t alloc_size) {
        return (void *)get_memory(alloc_size);
    }
    ...
    inline void free(T *value) {
        ...
        mem_array[pmeminfo->memory_check].free(pheader);
    }
